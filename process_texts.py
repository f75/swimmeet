        ...
        for i, line in enumerate(r):
            if i > 3:
                ...
                if line[0:3] == b'D01':
                    ...
                    file_record['event_date'] = line[80:89].strip()
                    # file_record['event_group'] = line[80:89]
                    # parse entry time
...
def best_time(x):
    ...
    if course in ('Y', 'S'):
        if course == x[2][-1]:
            add_drop = t3 - min_time

    return min_time, course, round(add_drop, 3)
...
if __name__ == '__main__':
    ...
    df['event_date'] = pd.to_datetime(df['event_date'], errors='coerce', format="%m%d%Y")
    df['date_formatted'] = df['event_date'].dt.strftime('%m/%d/%Y')
    ...
    df['result'], df['course'], df['drop_add'] =\
        zip(*df[['pre_end_time', 'result_time', 'entry_time']].apply(best_time, axis=1))
    ...
    df['aj'] = df.groupby(['event_mm', 'course','age', 'sex_code', 'event_dist', 'event_type'])['result'] \
        .rank(method='dense', ascending=True).fillna(0).astype(int)
