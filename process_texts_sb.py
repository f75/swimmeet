...
#%%
    ...
    df['dob_mm'] = df['dob'].map(lambda x: 100*x.year + x.month).fillna(0).astype(int)

    #SOMAR
    df['event_date'] = pd.to_datetime(df['event_date'], errors='coerce', format="%m%d%Y")
    df['ed_mm'] = df['event_date'].map(lambda x: 100*x.year + x.month).fillna(0).astype(int)
    #somar
    ...
    df['ak'] = df.groupby(['age', 'sex_code', 'course','event_dist', 'event_type'])['result']\
        .rank(method='dense',ascending=True).fillna(0).astype(int)
    df['aj'] = df.groupby(['ed_mm','age', 'sex_code', 'course','event_dist', 'event_type'])['result']\
        .rank(method='dense', ascending=True).fillna(0).astype(int)

    # Write final csv file
    df = df.sort_values(by=['event_date','age', 'sex_code', 'course','event_dist', 'event_type', 'ak','aj'],
